function [trfinal] = TablaVariablesCategArbolClassifTodosYears(ttp24h,K,nomInputVars,timetableFestivos) 
    %K= 12;
    %year =2007;
    primerYear = ttp24h.FECHA.Year(1,:);
    ultimoYear = ttp24h.FECHA.Year(end,:);
    trfinal=[];
    for dur=primerYear:ultimoYear
        [idx, ctrs,sumd] = KMeansYear(ttp24h,K,dur,nomInputVars);%aunque esto lo tengo que hacer por cada año

        years = ttp24h.FECHA.Year == dur;
        ...
        vecOrdenClusters=OrdenRespectoMasFrec(ttp24h,K,0,nomInputVars,dur);
%{
        if dur==ultimoYear
                vecOrdenClusters=UltimoYearOrdenRespectoMasFrec(infomat,K,0,nomInputVars,dur);
        else
            vecOrdenClusters=OrdenNextYearRespectoMasFrec(infomat,K,0,nomInputVars,dur);       
        end
%}
        ...
        tDiaSemana=array2table(weekday(tr.FECHA),'VariableNames',{'DiaSemana'});
        diaSemana = string({"Domingo","Lunes","Martes","Miercoles","Jueves","Viernes","Sabado","Festivo"});
        ...
        tDiaSemana=array2table(tDiaSemanacell','VariableNames',{'DiaSemana'});
        ...
        tEstaciones=array2table(tEstacioncell','VariableNames',{'Estacion'});
